...
def DualResNet_imagenet(pretrained_filename, pretrained=False):
    model = DualResNet(BasicBlock, [2, 2, 2, 2], num_classes=19, planes=32, spp_planes=128, head_planes=64, augment=True)
    if pretrained:
        pretrained_state = torch.load(pretrained_filename, map_location='cpu')['model_state_dict']
        model_dict = model.state_dict()
        pretrained_state_edited = {k[6:] if 'model' in k else k:v for k,v in pretrained_state.items()}
        model_dict.update(pretrained_state_edited)
        model.load_state_dict(model_dict, strict = False)

    return model

def get_seg_model(pretrained_filename, pretrained, **kwargs):
    model = DualResNet_imagenet(pretrained_filename, pretrained=pretrained)
    return model
